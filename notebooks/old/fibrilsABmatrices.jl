...
# Points in convex hull are ordered clockwise already
# Define boundary for constrained triangulation
# hullEdges = [(hullInds[end],hullInds[1])]
# for i=2:length(hullInds)
#     push!(hullEdges,(hullInds[i],hullInds[i-1]))
# end
# hullEdges = Set(hullEdges)

# pts = Tuple.(centroidLocations)
pts = deleteat!(Tuple.(centroidLocations),sort(hullInds))

boundary_nodes, pts = convert_boundary_points_to_indices(reverse(Tuple.([hull.vertices...,hull.vertices[1]])); existing_points=pts)

triangulation=triangulate(pts;boundary_nodes,delete_ghosts=false)
...
fig, ax, sc = voronoiplot(tessellation, strokecolor=:red, markersize=9)
# fig = Figure(resolution=(750,750)); ax=Axis(fig[1,1],aspect=DataAspect())
triplot!(ax, triangulation, strokewidth=1, strokecolor=(:black, 0.4))
