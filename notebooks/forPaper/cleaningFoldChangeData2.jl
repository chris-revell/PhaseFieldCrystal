...
indices = Int64[]
for (i,gene) in enumerate(subsetGeneNames)
    gene=="Try10" ? append!(indices,i) : nothing 
end
deleteat!(subsetGeneNames,indices)
geneLabels = Values_for_heatmap["A2:A107"][:,1]
deleteat!(geneLabels,indices)
...
for i in 1:length(indices)
    if fullGeneNames[indices[i]] == "Try10" 
        push!(indicesToRemove,i)
    end
end
...
proteinsForLater = copy(allProteins)
...
allProteinsRearranged[51:74] .= reverse(allProteins[51:74])
...
colorLabelsRearranged[51:74] .= reverse(colorLabels[51:74])
...
ax3 = Axis(fig[1,3], xticks=(1:5, string.(t)), yticks = (1:24, [rich(allProteinsRearranged[val]; color) for (val, color) in zip(51:74, colorLabelsRearranged[51:74])]))
...
heatmap!(ax3,rotr90(allHeatmapData[51:74,:]), colormap=:bwr, colorrange=(-maximum(abs.(allHeatmapData))+1, maximum(abs.(allHeatmapData))+1))
...
absoluteChangeDataFrameLogsOriginal = DataFrame(
    label = fill("",106), #geneLabels,
    gene_name = fill("",106), #subsetGeneNames, 
    E12_5 = zeros(Float64,106),#length(subsetGeneNames)),
    E13_0 = zeros(Float64,106),#length(subsetGeneNames)),
    E13_5 = zeros(Float64,106),#length(subsetGeneNames)),
    E14_0 = zeros(Float64,106),#length(subsetGeneNames)),
    E14_5 = zeros(Float64,106),#length(subsetGeneNames))
)

absoluteChangeDataFrameLogsOriginal[!,:] = Values_for_heatmap["A2:G107"]
...
absoluteChangeDataFrameOriginal = copy(absoluteChangeDataFrameLogsOriginal)
absoluteChangeDataFrameOriginal[!,3:7] = 2.0.^absoluteChangeDataFrameLogsOriginal[!,3:7]

allProteinsReference = copy(allProteins)

absoluteChangeDataFrame = filter(:gene_name=>n->n∈proteinsForLater, absoluteChangeDataFrameOriginal)
absoluteChangeDataFrameLogs = filter(:gene_name=>n->n∈proteinsForLater, absoluteChangeDataFrameLogsOriginal)
...
ax3 = Axis(fig2[1,3], xticks=(1:5, string.(t)), yticks = (1:24, [rich(allProteinsRearranged[val]; color) for (val, color) in zip(51:74, colorLabelsRearranged[51:74])]))
...
heatmap!(ax3,rotr90(allHeatmapData[51:74,:]), colormap=:batlow, colorrange=(minimum(allHeatmapData), maximum(allHeatmapData)))
...
display(fig2)
save(datadir("exp_pro","MSSpreadsheets","All proteins absolute change heatmap2.png"),fig2)
#save(datadir("exp_pro","MSSpreadsheets","All proteins fold change heatmap2.svg"),fig)
...
ax3 = Axis(fig3[1,3], xticks=(1:5, string.(t)), yticks = (1:24, [rich(allProteinsRearranged[val]; color) for (val, color) in zip(51:74, colorLabelsRearranged[51:74])]))
heatmap!(ax1,rotr90(allHeatmapData[1:25,:]), colormap=:batlow, colorrange=(minimum(allHeatmapData), maximum(allHeatmapData)))    
heatmap!(ax2,rotr90(allHeatmapData[26:50,:]), colormap=:batlow, colorrange=(minimum(allHeatmapData), maximum(allHeatmapData)))
heatmap!(ax3,rotr90(allHeatmapData[51:74,:]), colormap=:batlow, colorrange=(minimum(allHeatmapData), maximum(allHeatmapData)))
...
display(fig3)
save(datadir("exp_pro","MSSpreadsheets","logAbundancesHeatmap2.png"),fig3)
#save(datadir("exp_pro","MSSpreadsheets","All proteins fold change heatmap2.svg"),fig)
...
ax9 = Axis(fig4[1,1], xticks = (1:74, [rich(genenames[val]; color) for (val, color) in zip(1:74, colorLabels)]),xticklabelrotation=π/2)
barplot!(ax9,collect(1:length(genenames)),log2.(means),color=colorLabels)
grid = GridLayout(fig4[2,:])
# Box(grid[1,:], color = :black)
# Box(gd[i, 3], color = :gray90)
Label(grid[1, 1], justification = :center, "Protein colour labels:", fontsize=16, lineheight = 2.0)
Label(grid[1, 1+1], justification = :center, "$(unique(allLabels)[1])", color=colors[1], fontsize=16, lineheight = 2.0)
...
xlims!(0.5,74.5)
display(fig4)
save(datadir("exp_pro","MSSpreadsheets","timeAveragedAbundance2.png"),fig4)